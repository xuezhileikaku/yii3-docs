...
此文档定义与 [yiisoft/config](https://github.com/yiisoft/config) 一起使用的框架组的命名约定。
请注意,这不是配置文件的命名约定。
这些可以是任何内容,并通过 `composer.json` 映射到组名。

## 配置组名后缀

- "web" 后缀仅适用于 Web,即经典服务器 HTML 生成、REST、RPC 等。
- "console" 后缀适用于控制台。
- 如果没有后缀,它是"common"并且适用于 Web 和控制台。
...
- `di-providers` — 通用容器提供程序
- `di-providers-web` — Web 容器提供程序
- `di-providers-console` — 控制台容器提供程序
