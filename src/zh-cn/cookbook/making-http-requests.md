...
PHP-FIG（PHP 框架互操作性小组）定义了几个用于 HTTP 处理的 PSR 标准：

- **PSR-7**：用于请求和响应的 HTTP 消息接口
- **PSR-17**：用于创建 PSR-7 消息对象的 HTTP 工厂接口
- **PSR-18**：用于发送 PSR-7 请求和返回 PSR-7 响应的 HTTP 客户端接口

使用这些接口可确保您的代码与框架无关，并遵循既定的 PHP 标准。

## 安装

安装支持 PSR-18 的 Guzzle HTTP 客户端和 PSR-17 工厂：
...
Guzzle 支持中间件来处理跨领域关注点，例如身份验证、日志记录或重试：
...
3. **配置超时**：设置合理的连接和请求超时以防止请求挂起。

4. **记录请求**：使用中间件或手动日志记录来跟踪 API 调用以进行调试和监控。

5. **使用依赖注入**：通过 DI 容器注入 HTTP 客户端和工厂，而不是直接创建它们。

6. **在测试中模拟**：使用 Guzzle 的 MockHandler 或类似工具来测试 HTTP 客户端代码，而不发起真实的网络请求。
