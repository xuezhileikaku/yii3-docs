...
视图负责向最终用户展示数据。您给它一个带有一些占位符和表示逻辑的模板以及一些数据。视图将数据传递给模板并执行模板逻辑。最终结果准备好传递给最终用户，无论是浏览器、要下载的文件、要发送的电子邮件还是其他内容。
...
在 Yii3 中，视图通常是包含表示逻辑和 HTML 标记的 PHP 文件。视图系统提供了一种灵活的方式来组织表示层，并支持布局和局部视图等功能。除了使用纯 PHP 模板，您还可以利用[模板引擎（如 Twig）之一](template-engines.md)。
...
对于 Web 应用程序，您还应该安装 `yiisoft/yii-view-renderer` 包，它提供 [PSR-7](https://www.php-fig.org/psr/psr-7/) 兼容性和 Web 特定功能：
...
视图模板文件包含表示逻辑。在 `yiisoft/app` 模板中，视图文件通常与其控制器存储在一起（例如 `src/Web/Echo/Action.php`）。这是一个简单的视图文件示例 `src/Web/Echo/template.php`：
...
这里 `$message` 是视图数据，当您使用 `ViewRenderer` 渲染模板时传递。例如，`src/Web/Echo/Action.php`：
...
`render()` 方法的第一个参数是模板文件的路径。在 `yiisoft/app` 中，模板文件通常与其操作存储在一起。结果准备好渲染到浏览器，所以我们立即返回它。

## 使用布局

大多数 Web 应用程序为所有页面使用通用布局。在 `yiisoft/app` 模板中，布局存储在 `src/Web/Shared/Layout/Main/` 目录中。您可以在 `config/common/params.php` 中设置默认布局：
...
典型的布局文件（如 `src/Web/Shared/Layout/Main/layout.php`）如下所示：
...
`$aliases` 引用 [别名组件](../concept/aliases.md)，用于获取应用程序在服务器上的基本 URL。

`$this` 是视图的实例，我们用它来获取页面标题和输出资源。标准的和自定义的。

通过 `$assetManager->register(MainAsset::class);` 我们注册一个资源，定义要包含到页面中的 `css`。它在首次使用时自动复制到 `public/assets`。对于单个 CSS 文件来说不是很有用，但随着资源数量的增长会变得方便。

> [!IMPORTANT]
> 在纯 PHP 模板中没有输出被编码，您不应忘记使用 `Html::encode()` 来防止 XSS 安全漏洞。

更多关于布局中可用的内容可以在 [yiisoft/view](https://github.com/yiisoft/view) 文档中阅读。
...
您可以使用 `$this->render()` 方法从视图内渲染其他视图：
...
块允许您在一个视图中定义内容并在另一个视图中显示，通常在布局中：
