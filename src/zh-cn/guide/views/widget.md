...
小部件是可重用的、自包含的组件，封装了复杂的 HTML 生成逻辑。它们提供了一种创建可配置 UI 元素的干净方法，这些元素可以在不同的视图和应用程序中使用。Yii3 通过 `yiisoft/widget` 包提供了一个灵活的小部件系统。
...
小部件是一个 PHP 类，它扩展抽象 `Widget` 类并实现一个 `render()` 方法，该方法返回 HTML 内容作为字符串。

### 小部件工厂

`WidgetFactory` 负责创建小部件实例，可以通过 DI 容器注入依赖关系。

### 小部件配置

小部件可以通过属性和方法进行配置，使其高度可配置和可跨不同上下文重用。
...
小部件为在 Yii3 应用程序中创建可重用、可配置的 UI 组件提供了强大的方法。它们帮助保持清晰的关注点分离，并使您的视图更易于维护和测试。
