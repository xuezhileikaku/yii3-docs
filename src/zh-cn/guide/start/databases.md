...
Yii 不会强制要求你的应用程序使用特定的数据库或存储。有很多方法可以使用关系型数据库：
...
在本指南中，我们将专注于使用 Yii DB 处理关系型数据库。我们将使用 PostgreSQL 来实现一个简单的 CRUD（创建、读取、更新、删除）。

## 安装 PostgreSQL

你需要安装 PostgreSQL。如果你不想使用 Docker，
[从官方网站获取安装程序](https://www.postgresql.org/download/)，安装它并创建一个数据库。

如果你使用 Docker，这会稍微简单一些。修改 `docker/dev/compose.yml`：
...
请注意，我们添加了 `depends_on`，以便应用程序等待数据库启动。

此外，我们需要 `pdo_pgsql` 扩展来与 PostgreSQL 通信。你可以在 `php.ini` 中本地启用它。
如果使用 Docker，请检查 `docker/Dockerfile` 并在 `install-php-extensions` 列表中添加 `pdo_pgsql`。然后使用 `make build && make down && make up` 重新构建 PHP 镜像。
...
在 Docker 网络内，`db` 主机会自动解析。

对于不使用 Docker 的本地安装，Dsn 中的主机将是 `localhost`。你需要调整其余部分以匹配你配置数据库的方式。

## 创建和应用迁移

对于应用程序的初始状态和后续的数据库更改，使用迁移是一个好主意。这些是创建数据库更改的文件。应用的迁移会在数据库中跟踪，允许我们了解当前状态以及哪些迁移尚未应用。
...
创建一个目录来存储迁移 `src/Migration`，就在项目根目录中。将以下配置添加到 `config/common/params.php`：
...
现在你可以使用 `make yii migrate:create page` 来创建一个新的迁移。对于我们的示例，我们需要一个带有一些列的 `page` 表：
...
请注意，我们使用 UUID 作为主键。我们将自己生成这些 ID，而不是依赖数据库，因此我们需要一个额外的 composer 包。

```shell
make composer require ramsey/uuid
```

虽然存储空间比使用 int 稍大，但使用这种 ID 的工作流程是有益的。由于你自己生成 ID，因此你可以定义一组相关数据并在单个事务中保存它。在代码中定义这组数据的实体通常被称为"聚合"。

使用 `make yii migrate:up` 应用迁移。

## 实体

现在你有了一个表，是时候在代码中定义一个实体了。创建 `src/Web/Page/Page.php`：
...
现在我们有了实体，我们需要一个地方来保存实体的方法、删除它以及选择单个页面或多个页面。
...
在这个仓库中，既有获取数据的方法，也有执行插入或更新的 `save()` 方法。数据库以数组形式返回原始数据，但我们的仓库会自动从这些原始数据创建实体，以便我们后续操作类型化数据。
...
在此视图中，我们有一个提交删除页面请求的表单。使用 `GET` 处理也很常见，但这是非常错误的。由于删除会更改数据，因此需要通过非幂等的 HTTP 方法之一来处理。我们在示例中使用 POST 和表单，但也可以使用 DELETE 和使用 JavaScript 发出的异步请求。稍后可以正确设置按钮样式，使其看起来与"Edit"相似。
...
在上面，我们在 URL 中为新页面使用了一个特殊的 slug，因此 URL 看起来像 `http://localhost/pages/new`。如果页面不是新的，我们会用数据库中的数据预填充表单。与我们在[使用表单](forms.md)中所做的一样，我们处理表单提交。成功保存后，我们重定向到页面视图。
...
请注意，我们在 `/pages` 前缀下将所有与页面相关的路由分组到一个组中。这是一种既方便不重复自己，又可以为整个组添加一些额外中间件（如身份验证）的好方法。
