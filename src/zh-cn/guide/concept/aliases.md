...
你可以使用别名来表示文件路径或 URL，这样你就不必在项目中
硬编码绝对路径或 URL。别名必须以 `@` 字符开头，以便与普通文件路径和 URL 区分开。定义时没有前导 `@` 的别名将加上 `@` 字符前缀。

默认的 Yii 应用程序在 `config/params.php` 中预定义了一些别名。例如，别名 `@public` 表示
Web 根路径；`@baseUrl` 表示当前运行的 Web 应用程序的基本 URL。

## 定义别名 <span id="defining-aliases"></span>

你可以通过应用程序的 `config/params.php` 定义别名：
...
> [!NOTE]
> 被别名的文件路径或 URL*不一定*引用现有文件或资源。

给定一个已定义的别名，你可以通过附加斜杠 `/` 后跟一个或多个路径段来派生新别名。
例如，`@foo` 是根别名，而 `@foo/bar/file.php` 是派生别名。

你可以使用另一个别名（根或派生）来定义别名：

```php
'@foobar' => '@foo/bar',
```

`yiisoft/aliases` 参数初始化来自 [`yiisoft/aliases` 包](https://github.com/yiisoft/aliases)的 `Aliases` 服务。
你可以在运行时使用该服务设置额外的别名：
...
最好在配置级别解析别名，以便服务将 URL 和路径作为随时可用的字符串获取：
...
你可以使用 `Aliases` 服务将别名或派生别名解析为其代表的文件路径或 URL：
...
派生别名表示的路径/URL 通过用派生别名中相应的路径/URL 替换根别名部分来确定。

> [!NOTE]
> `get()` 方法不检查结果路径/URL 是否引用现有文件或资源。


根别名也可以包含斜杠 `/` 字符。`get()` 方法
足够智能，可以判断别名的哪一部分是根别名，从而正确确定
相应的文件路径或 URL：
...
如果 `@foo/bar` 未定义为根别名，则最后一条语句将显示 `/path/to/foo/bar/file.php`。
...
- `@root` - 当前运行应用程序的基本目录。
- `@assets` - 应用程序发布资产的公共目录。
- `@assetsUrl` - 具有已发布资产的基本目录的 URL。
- `@baseUrl` - 当前运行的 Web 应用程序的基本 URL。默认为 `/`。
- `@npm` - node 包目录。
- `@bower` - bower 包目录。
- `@vendor` - Composer 的 `vendor` 目录。
- `@public` - 应用程序的公共可访问目录，其中包含 `index.php`。
- `@runtime` - 当前运行应用程序的运行时路径。默认为 `@root/runtime`。
- `@layout` - 包含布局的目录。
- `@resources` - 包含视图、资产源和其他资源的目录。
- `@views` - 应用程序视图模板的基本目录。
