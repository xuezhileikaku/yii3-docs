...
伪随机数据在许多情况下都很有用。例如，当通过电子邮件重置密码时，您需要生成一个令牌，将其保存到数据库，并通过电子邮件发送给最终用户，这将允许他们证明对该帐户的所有权。该令牌必须是唯一且难以猜测的，否则攻击者可能会预测令牌的值并重置用户的密码。
...
Yii 提供了方便的辅助函数，可以使用密钥加密/解密数据。数据通过加密函数传递，以便只有拥有密钥的人才能解密它。例如，您需要在数据库中存储一些信息，但您需要确保只有拥有密钥的用户才能查看它（即使有人破坏了应用程序数据库）：
...
在某些情况下，您需要验证您的数据是否没有被第三方篡改或以某种方式损坏。Yii 提供了一种通过 MAC 签名来确认数据完整性的方法。

`$key` 应该存在于发送和接收两侧。在发送侧：
...
掩码令牌有助于通过随机化每次请求输出的令牌来减轻 BREACH 攻击。应用了一个随机掩码，使字符串始终唯一。
